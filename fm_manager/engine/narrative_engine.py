...
from fm_manager.engine.match_engine_markov import MatchEvent
...
class MatchNarrativeGenerator:
    ...
    def _generate_headline(self, match: Match, home: Club, away: Club) -> str:
        ...
        home_goals = (
            ...
            else (match.home_goals if hasattr(match, "home_goals") else 0)
        )
        away_goals = (
            match.away_score
            if hasattr(match, "away_score")
            else (match.away_goals if hasattr(match, "away_goals") else 0)
        )
...
class NarrativeEngine:
    ...
    def generate_match_report(
        ...
        return self.match_gen.generate_match_narrative(match, events, home_club, away_club, use_llm)
