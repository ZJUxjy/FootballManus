"""Adapter to use database Player models with match engine.
...
IMPROVEMENTS (V2):
- Prioritize database technical attributes directly when available
- Calculate from position ratings ONLY when database attributes are missing
- Preserve actual technical attribute values from database (e.g., shooting=95, passing=92)
"""
...
class AdaptedPlayer:
    ...
    This adapter prioritizes database attributes in this order:
    1. Database technical attributes (shooting, passing, dribbling, etc.) - PRIMARY SOURCE
    2. Database current_ability - used as base for calculations
    3. Position ratings from CSV - used as fallback when above are missing

    V2 Changes:
    - For database players: Use technical attributes DIRECTLY without recalculating
    - For CSV players: Still use position ratings as fallback
    - Better fallback logic: If attribute value is 0 or None, then calculate from CA
    """
    ...
    def get_role_bonus(self, action_type: str) -> float:
        ...
        role_bonus = {
            ...
            (PlayerRole.BALL_WINNING_DEFENDER, "passing"): 1.08,
            (PlayerRole.BOX_TO_BOX, "passing"): 1.10,
            ...
            # Physical bonuses
            (PlayerRole.STRIKER, "pace"): 1.10,
            (PlayerRole.WINGER, "pace"): 1.08,
            (PlayerRole.COMPLETE_FORWARD, "pace"): 1.05,
        }

        return role_bonus.get((self.assigned_role, action_type), 1.0)

    def _init_from_db_player(self, player: Player):
        """Initialize from database Player model with full technical attributes.

        V2: Directly uses database technical attributes without recalculating them.
        """
        self.id = player.id
        ...
        # ========================================
        # V2: USE DATABASE ATTRIBUTES DIRECTLY
        # ========================================
        # Priority 1: Database technical attribute if > 0
        # Priority 2: Calculate from CA only if attribute is 0 or None

        # Physical attributes
        self.pace = self._get_attr_or_calc(player, "pace", 0.85)
        self.acceleration = self._get_attr_or_calc(player, "acceleration", 0.75)
        self.stamina = self._get_attr_or_calc(player, "stamina", 0.90)

        # Technical attributes
        self.shooting = self._get_attr_or_calc(player, "shooting", 0.9)
        self.passing = self._get_attr_or_calc(player, "passing", 0.85)
        self.dribbling = self._get_attr_or_calc(player, "dribbling", 0.85)
        self.crossing = self._get_attr_or_calc(player, "crossing", 0.7)
        self.first_touch = self._get_attr_or_calc(player, "first_touch", 0.8)

        # Defensive/Mental attributes
        self.tackling = self._get_attr_or_calc(player, "tackling", 0.6)
        self.marking = self._get_attr_or_calc(player, "marking", 0.6)
        self.positioning = self._get_attr_or_calc(player, "positioning", 0.85)
        self.vision = self._get_attr_or_calc(player, "vision", 0.8)
        self.decisions = self._get_attr_or_calc(player, "decisions", 0.8)

        # Goalkeeping attributes
        self.reflexes = self._get_attr_or_calc(player, "reflexes", 0.85)
        self.handling = self._get_attr_or_calc(player, "handling", 0.85)
        self.kicking = self._get_attr_or_calc(player, "kicking", 0.75)
        self.one_on_one = self._get_attr_or_calc(player, "one_on_one", 0.85)
    ...
    def _init_from_data_full(self, player_data: PlayerDataFull):
        ...
        V2: Uses position ratings as fallback, but with better fallback logic.
        """
        ...
        # Map position ratings to general attributes (legacy calculation for CSV)
    ...
    def _get_attr_or_calc(
        self, player: Player, attr_name: str, ability_multiplier: float
    ) -> int:
        """Get attribute from database or calculate from CA.

        Priority:
        1. Database attribute if it exists and > 0 (V2: DIRECT USE)
        2. Calculate from current_ability ONLY if attr is 0 or None

        This ensures that for database players with full technical attributes,
        those actual values are used directly instead of recalculating them.
        """
        attr_value = getattr(player, attr_name, 0)

        # Check if database attribute is valid (> 0)
        if attr_value and attr_value > 0:
            # V2: Use database attribute directly
            return attr_value

        # Fallback: calculate from current_ability only
    ...
    def _calculate_attributes_from_ratings(self, player_data: PlayerDataFull):
        ...
        V2: Still used as fallback for CSV players, but with better multipliers.
        """
    ...
    def get_position_rating(self) -> float:
        """Get position-specific rating for this player."""
...
class MatchSimulatorWithRealData:
    ...
    def get_premier_league_clubs(self) -> list[ClubDataFull]:
