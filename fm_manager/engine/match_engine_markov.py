...
class MatchState:
    ...
    home_player_stats: Dict[str, PlayerMatchState] = field(default_factory=dict)
    away_player_stats: Dict[str, PlayerMatchState] = field(default_factory=dict)
...
class MarkovMatchEngine:
    """Legacy base engine - kept for compatibility."""

    def __init__(self, random_seed: Optional[int] = None):
        self.rng = random.Random(random_seed)

    def simulate(self, home_lineup: list, away_lineup: list, **kwargs) -> MatchState:
        """Basic simulation - overridden by EnhancedMarkovEngine."""
        return MatchState(home_lineup=home_lineup, away_lineup=away_lineup)
    ...
    def _update_stats(self, match_state: MatchState, event: MatchEvent, game_state: GameState):
        ...
        if event.event_type == MatchEventType.PASS:
            if game_state.possession == Possession.HOME:
                match_state.home_passes += 1
            else:
                match_state.away_passes += 1
        elif event.event_type == MatchEventType.SHOT:
            if game_state.possession == Possession.HOME:
                match_state.home_shots += 1
            else:
                match_state.away_shots += 1
        elif event.event_type == MatchEventType.SHOT_ON_TARGET:
            if game_state.possession == Possession.HOME:
                match_state.home_shots += 1
        ...
        elif event.event_type == MatchEventType.GOAL_HOME:
            match_state.home_goals += 1
            match_state.home_shots += 1
            match_state.home_shots_on_target += 1
        elif event.event_type == MatchEventType.GOAL_AWAY:
