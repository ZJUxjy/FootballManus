...
async def test_engine():
    ...
    async with get_db_session() as session:
        ...
        print(
            f"Home shots: {match_state.home_shots} (on target: {match_state.home_shots_on_target})"
        )
        print(
            f"Away shots: {match_state.away_shots} (on target: {match_state.away_shots_on_target})"
        )
