...
def simulate_season(enable_rotation: bool = False):
    ...
    if enable_rotation:
        squad_builders = {
            club.id: ClubSquadBuilder(club, enable_rotation=True) for club in premier_league
        }
    else:
        squad_builders = {}
    ...
    for idx, (home, away) in enumerate(fixtures, 1):
        ...
        if enable_rotation:
            ...
            opponent_strength_for_home = sum(p.current_ability for p in away.players) / len(
                away.players
            )
            opponent_strength_for_away = sum(p.current_ability for p in home.players) / len(
                home.players
            )
    ...
    print("=" * 80)
    print(
        f"{'Pos':<4} {'Club':<25} {'P':<3} {'W':<3} {'D':<3} {'L':<3} {'GF':<4} {'GA':<4} {'GD':<5} {'Pts':<5}"
    )
    print("-" * 80)
    ...
    for i, team in enumerate(league_table, 1):
        ...
        print(
            f"{i:<4} {team.team_name:<23} {team.matches_played:<3} {team.wins:<3} "
    ...
    for i, player in enumerate(tracker.get_top_scorers(15), 1):
        ...
        print(
            f"{i:<3} {player.player_name:<25} {player.team_name:<20} "
    ...
    print(
        f"  Goals: {top_scorer.goals} | Assists: {top_scorer.assists} | Avg Rating: {top_scorer.avg_rating:.2f}"
    )
    print(f"  Shots: {top_scorer.shots} (Shot accuracy: {top_scorer.get_shot_accuracy():.1f}%)")
    print(
        f"  Passes: {top_scorer.passes_attempted} (Pass accuracy: {top_scorer.get_pass_accuracy():.1f}%)"
    )
    ...
    if top_scorer.position == "GK":
        print(
            f"  Saves: {top_scorer.saves} | Clean Sheets: {top_scorer.clean_sheets} | Goals Conceded: {top_scorer.goals_conceded}"
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Simulate a football season with optional rotation system"
    )
